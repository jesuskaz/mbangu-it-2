    ...
    function import()
    {
        ...
        if (isset($_FILES['file']['name'])) {
            ...
            if (!(($extension == 'xlsx' || $extension == 'xls' || $extension == 'csv') && in_array($_FILES['file']['type'], $file_mimes))) {
                die(json_encode(['status' => false, 'message' => "Veuillez selectionner le fichier correct : .xlsx, .xls, .csv"]));
            }
        } else {
        ...
        if (!empty($_FILES['file']['name'])) {
            ...
            $flag = 0;
            $createArray = array('NOM', 'POSTNOM', 'PRENOM', 'MATRICULE', 'ADRESSE');
            $makeArray = array('NOM' => "NOM", 'POSTNOM' => 'POSTNOM', 'PRENOM' => 'PRENOM', 'MATRICULE' => 'MATRICULE', 'ADRESSE' => 'ADRESSE');
            $SheetDataKey = array();
