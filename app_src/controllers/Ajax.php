...
class Ajax extends CI_Controller
{
    ...
    function chart_data_3()
    {
        $this->checktype('banque');
        ...
        $this->db->where('frais_ecole.idbanque', $idbanque);
    ...
    function article_a()
    {
        ...
        if (strlen($titre) > 45) {
            $rep['status'] = false;
            $rep['message'] = 'Le titre doit avoir 45 caractères max.';
            echo json_encode($rep);
            exit;
        }
        ...
        if (isset($_FILES['file']['name'])) {
            ...
            for ($i = 0; $i < $tot; $i++) {
                if (!empty($_FILES['file']['name'][$i])) {
                    // $tt = "d" . (time() + $i) . ".pdf";
                    $path =  "upload/article/";
                    $f = $_FILES['file']['name'][$i] ?? '';
                    $f = explode('.', $f);
                    if (count($f) >= 2) {
                        $exe = end($f);
                        $f = time() . rand(1, 10000) . $i . '.' . $exe;
                    } else {
                        $f = '';
                    }
                    $titre_orignine = $_FILES['file']['name'][$i];
                    ...
                    if ($this->upload->do_upload('ufile')) {
                        ...
                        $rep['status'] = true;
                        $total['total_upload'][] = $titre_orignine;
                    } else {
                        $total['total_noupload'][] = $titre_orignine;
            ...
            if ($ok) {
                if ($type == 'ecole') {
                    $idecole = $this->session->ecole_session;
                    $this->db->insert('article_ecole', ['idecole' => $idecole, 'titre' => $titre, 'iddevise' => $devise, 'description' => $description, 'prix' => $prix, 'image' => $nomFichier]);
                } else if ($type == 'univ') {
                    $id = $this->session->universite_session;
                    $this->db->insert('article_universite', ['iduniversite' => $id, 'titre' => $titre, 'iddevise' => $devise, 'description' => $description, 'prix' => $prix, 'image' => $nomFichier]);
                }
                $rep['message'] = 'Article ajouté.';
            }
            if (count($total['total_noupload'])) {
                $ch = 'les fichiers suivant n\'ont pas été téléchargés : <em>';
                foreach ($total['total_noupload'] as $tu) {
                    $ch .= $tu . ' ';
                }
                $ch .= '</em>';
                $rep['message2'] = $ch;
            ...
            $rep['message'] = 'Aucune image n\'a été séléctionnée.';
        }
        echo json_encode($rep);
    }
    ...
    function article_u()
    {
        ...
        if (!empty($_FILES['file']['name'][0])) {
            ...
            if ($ok) {
                if ($type == 'ecole') {
                    $art = $this->db->where('idarticle', $idarticle)->get('article_ecole')->result()[0];
                    foreach (explode(',', $art->image) as $ia) {
                        @unlink(trim($ia));
                    }
                    $idecole = $this->session->ecole_session;
                    $this->db->update(
                        'article_ecole',
                        ['idecole' => $idecole, 'titre' => $titre, 'iddevise' => $devise, 'description' => $description, 'prix' => $prix, 'image' => $nomFichier],
                        ['idarticle' => $idarticle]
                    );
                } else if ($type == 'univ') {
                    $art = $this->db->where('idarticle', $idarticle)->get('article_universite')->result()[0];
                    foreach (explode(',', $art->image) as $ia) {
                        @unlink(trim($ia));
                    }
                    $id = $this->session->universite_session;
                    $this->db->update(
                        'article_universite',
                        ['iduniversite' => $id, 'titre' => $titre, 'iddevise' => $devise, 'description' => $description, 'prix' => $prix, 'image' => $nomFichier],
                        ['idarticle' => $idarticle]
                    );
                }
                $rep['message'] = 'Article mis à jour.';
            }
            if (count($total['total_noupload'])) {
                $ch = 'les fichiers suivant n\'ont pas été téléchargés : <em>';
                foreach ($total['total_noupload'] as $tu) {
            ...
            if ($type == 'ecole') {
                ...
                $this->db->update(
                    'article_ecole',
                    ['idecole' => $idecole, 'titre' => $titre, 'iddevise' => $devise, 'description' => $description, 'prix' => $prix],
                    ['idarticle' => $idarticle]
                );
            } else if ($type == 'univ') {
                $id = $this->session->universite_session;
                $this->db->update(
                    'article_universite',
                    ['iduniversite' => $id, 'titre' => $titre, 'iddevise' => $devise, 'description' => $description, 'prix' => $prix],
                    ['idarticle' => $idarticle]
                );
            }
            $rep['message'] = 'Article mis à jour.';
